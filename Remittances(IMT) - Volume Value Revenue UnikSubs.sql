select count(distinct Mobile_Number) unique_subs,count(distinct Transfer_Id) Volume,sum(Transaction_Value) value,
...
AND mti.transfer_date >= to_date('01/05/2021','dd/mm/yyyy') AND mti.transfer_date < to_date('19/05/2021','dd/mm/yyyy')+1
...
AND mti.party_id in (select a.user_id 
                                 ...
                                 --and a.status='Y'
                                 and cat.category_code = a.category_code
                                 and w.user_grade in ('REMAGENT0','REMAGENT1','REMAGENT','REMAGENT1')
                                 and w.payment_type_id = 12)
